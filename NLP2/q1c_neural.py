...
def forward(data, label, params, dimensions):
    ...
    z = sigmoid(np.dot(data, W1) + b1)    # hidden layer: sigmoid(Wx + b)
    output = softmax(np.dot(z, W2) + b2)    # output layer: softmax(Wz + b)
...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    ### YOUR CODE HERE: forward propagation
    z = sigmoid(np.dot(data, W1) + b1)
    y_hat = softmax(np.dot(z, W2) + b2)
    cost = -np.sum(labels * np.log(y_hat))
    ### END YOUR CODE

    ### YOUR CODE HERE: backward propagation
    err = y_hat - labels
    gradb2 = np.sum(err, axis=0)
    gradW2 = np.dot(z.T, err)
    delta = np.dot(err, W2.T) * sigmoid_grad(z)
