...
def replace_word(word):
    ...
    """
    ### YOUR CODE HERE 
    # word-replacement is adapted from the paper by M.Bikel et al.

    if word.isnumeric() and len(word)==4:
         return "fourDigitNum"

    if bool(re.search('[0-9]', word)) and bool(re.search('[a-zA-Z]', word)):
        return "containsDigitAndAlpha"

    dig= re.compile("\d") 
    if bool(dig.match(word)) and '/' in word:
    ...
    if re.search(r'(ating\b|ate\b|ize\b|fy\b|izes\b|ated\b|ates\b|fying\b)',word) or word.startswith('mis') or word.startswith('ob') or word.startswith('dis'):
        return '_verbLike_'

    if re.search(r'(ity\b|hood\b|ion\b|ty\b|ics\b|ment\b|ence\b|ness\b|age\b|ist\b|ism\b|logy\b|ice\b|sion\b|tion\b|ency\b|esis\b|osis\b|cian\b|acy\b|ship\b|dom\b|ance\b)', word):
        return '_nounLIKE_'

    if re.search(r'(less\b|able\b|ables\b|ful\b|ish\b|ic\b|al\b|ive\b)', word) or word.startswith('en') or word.startswith('im') or word.startswith('in') or word.startswith('ir') or word.startswith('anti'):
        return '_adjLike_'
    ...
    if word.lower() in {'which', 'whence','who', 'when', 'how', 'why', 'where', 'what', 'whose', 'whom'}:
        return '_wh_'

    if word.lower() in {'so', 'yet', 'or', 'nor', 'but', 'either', 'and'}:
        return '_conj_'

    if word.endswith('er') or word in {'worse', 'more', 'less'}:
        return '_comp_'
    ...
    if word.lower() in {'this', 'such', 'these', 'rather','that', 'those'}:
        return "_det_"
    ...
    if word.lower() in {'it', 'i', 'you', 'she', 'he'}:
        return '_ppronoun_'
