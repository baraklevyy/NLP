...
class NerBiLstmModel(torch.nn.Module):
    ...
    def __init__(self, helper, config, pretrained_embeddings):
        ...
        self.embedding = torch.nn.Embedding(*pretrained_embeddings.shape,
                                              _weight=pretrained_embeddings)
        self.bilstm = torch.nn.LSTM(input_size=self.config.embed_size*self.config.n_features, hidden_size=self.config.hidden_size//2,\
                                batch_first=True, bidirectional=True)
        self.linear = torch.nn.Linear(self.config.hidden_size, self.config.n_classes)
    ...
    def forward(self, sentences):
        ...
        x = self.embedding(sentences.to(torch.int64))
        x = self._dropout(x)
...
class Trainer(TrainerBase):
    ...
    def _batch_loss(self, tag_probs, labels, masks):
        ...
        masked_labels = (labels * masks).type(torch.long)
        masks_unsqueezed = masks.unsqueeze(dim=2)
        masked_tag_probs = tag_probs.masked_fill(~masks_unsqueezed, 0)
...
class DataPreprocessor(BaseDataPreprocessor):
    def pad_sequences(self, examples):
        ...
        Example: for the (sentence, labels) pair: [[4,1], [6,0], [7,0]], [1, 0, 0], 
        ...
        for sentence, labels in examples:
            ...
            padded_size = max_length-len(sentence)
            padded_sentence = sentence + [zero_vector]*padded_size if padded_size >= 0 else sentence[:padded_size]
            padded_labels = labels + [zero_label]*padded_size if padded_size >= 0 else labels[:padded_size]
            _masking_sequence = ([True]*len(sentence) + [False]*padded_size) if padded_size >= 0 else [True]*len(padded_sentence)
            ret.append((padded_sentence, padded_labels, _masking_sequence))
            ### END YOUR CODE
        return ret
